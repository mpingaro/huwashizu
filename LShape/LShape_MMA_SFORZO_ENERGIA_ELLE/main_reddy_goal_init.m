...
VOLFRAC = 0.35;
VMAX = (base*height+base/2*height)*VOLFRAC;

%% GEOMETRY: LA ELLE DEVE ESSERE COMPOSTA DA TRE QUADRATI OGNUNO MESHATO CON NUMERO PARI
dx = base/ndx;
dy = height/ndy;
% Mesh 
N1 = (ndx+1)*(ndy+1); N2 = (ndx-1)*(ndy-1);
N3 = (ndx/2+1)*(ndy+1); N4 = (ndx/2-1)*(ndy-1); 
[coordinates, ~]=CoordinatesType2(ndx,ndy,dx,dy);
cospig1 = coordinates(1:N1,:);
cocent1 = coordinates(N1+1:end,:);
[coordinates2, ~]=CoordinatesType2(ndx/2,ndy,dx,dy);
coordinates2(:,1) = coordinates2(:,1) + base/2;
coordinates2(:,2) = coordinates2(:,2) + height;
cospig2 = coordinates2(1:N3,:); cospig2 = cospig2(ndx/2+2:end,:);
...
[element2,nelem2]=ElementType2(ndx/2,ndy);
element(:,1) = element(:,1)+(ndx/2+1)*ndy;
element2(:,2:3) = element2(:,2:3)+((ndx+1)*ndy+ndx/2);
...
LX2 = [2*ndx+1:2*(ndx+1):2*(ndx+1)*(ndy+1)-1 ...
    2*(ndx+1)*(ndy+1)-1+2*(ndx/2+1):2*(ndx/2+1):2*nnodspig-1]; LY2 = LX2+1;
LX3 = sort(2*nnodspig-1:-2:(2*nnodspig-1-2*ndx/2)); LY3 = LX3+1;
LX4 = sort(LX3(1):-2*(ndx/2+1):2*(ndx+1)*(ndy+1)-2*ndx/2-1); LY4 = LX4+1;
LX5 = 2*(ndx+1)*ndy+1:2:min(LX4); LY5 = LX5+1;
LX6 = 1:2*(ndx+1):2*(ndx)*(ndy+1)-3; LY6=LX6+1;
...
NMX2 = LX2(ndy+1);     NMY2 = NMX2 + 1;
NMX3 = LX3(ndx/2/2+1); NMY3 = NMX3 + 1;
NMX4 = LX4(ndy/2+1);   NMY4 = NMX4 + 1;
NMX5 = LX5(ndx/2/2+1); NMY5 = NMX5 + 1;
NMX6 = LX6(ndy/2+1);   NMY6 = NMX6 + 1;
