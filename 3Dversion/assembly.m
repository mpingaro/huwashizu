...
for k = 1:nelem
    P(1,[1 2 3]) = coordinates(element(k,1),[1 2 3]);
    P(2,[1 2 3]) = coordinates(element(k,2),[1 2 3]);
    P(3,[1 2 3]) = coordinates(element(k,3),[1 2 3]);
    P(4,[1 2 3]) = coordinates(element(k,4),[1 2 3]);
    [AELEM,BELEM,KELEM,MELEM,WELEM,DELEM] = reddy_3Delement(P,lambda,mu);
    load = body_load(P,g);
    % ASSEMBLY A, B, W;
    for i=1:15
        for j=1:15
