...
for i = 1:size(weight,2) % Cycle on gauss points --> Da ottimizzare.
   ...
   gradu(:,4) = DFF*[y-2*x*y-y*y; x-2*x*y-x*x]*27;
   
   %% Matrix:  A( epsi(u),apsi(v) ) Stored full matrix
   % Shape functions epsilon(u)
   epsi(1,:) = [gradu(1,1), 0, gradu(2,1)/2];
   epsi(2,:) = [0, gradu(2,1), gradu(1,1)/2];
   epsi(3,:) = [gradu(1,2), 0, gradu(2,2)/2];
   epsi(4,:) = [0, gradu(2,2), gradu(1,2)/2];
   epsi(5,:) = [gradu(1,3), 0, gradu(2,3)/2];
   epsi(6,:) = [0, gradu(2,3), gradu(1,3)/2];
   epsi(7,:) = [gradu(1,4), 0, gradu(2,4)/2];
   epsi(8,:) = [0, gradu(2,4), gradu(1,4)/2];   
   for nb_i=1:8
       for nb_j=1:8
